...
내용 기준: 2026-09-16. Shotloom 사례 공개 기록은 [v1.1.0](releases/v1.1.0.md), 후속 인터뷰와 LLM 협업 개발 반영 기록은 [v1.1.1](releases/v1.1.1.md)에 있다. 현재 배포 버전은 [GitHub Releases](https://github.com/tomlim2/ta-portfolio/releases)에서 확인한다.
...
| [Character System](projects/character-system.html) | 아티스트의 등록·색상 변형·결과 확인과 웹 VRM의 Studio 변환을 연결 | NPR: 캐릭터 룩 / PMX to VRM: 별도의 변환 R&D |
...
**UE5 Profiling의 조건과 협업:** 측정 환경은 Unreal Engine 에디터다. GPU 모델·해상도·정확한 실행 모드는 미확인이며 배포 환경의 성능으로 쓰지 않는다. 처음 TA팀이 하던 프로파일링을 Notion으로 배경·라이팅 팀에 공유해 라이트·머티리얼 종류, 텍스처 크기, 폴리곤 수를 자체 점검하도록 했다. 룩을 유지하면서 해결하기 어려운 부분은 TA팀에 요청해 커스터마이징하거나 더 저렴한 표현을 찾았다. 팀 전체의 작업을 본인의 단독 성과로 표현하지 않는다.
...
- **LLM 협업 개발·Knitten:** Claude·OpenAI 도구를 활용하는 4명이 하나의 제품을 개발했다. 핵심 기여는 변경 목적·범위·책임 경계·검증 상태를 서로 이해할 수 있게 하는 가드레일과 문서화다. 이를 지원하는 하네스로 Knitten 공통 코어와 Shotloom 어댑터, 단계별 참조·체크포인트, 설치·Git·CI 검사 도구를 구성했다. Knitten의 직접 사용자는 본인이며, 동료에게는 구현 맥락·검증 기록·코드가 이어졌다. 4명 모두가 Knitten을 실행한 것으로 확대하지 않는다. IK 인계는 공동 개발의 한 예시다.

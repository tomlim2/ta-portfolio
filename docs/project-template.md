...
- 푸터는 메인과 동일한 전체 링크·1400px 최대 너비·여백을 유지한다. 이메일·이력서·GitHub·LinkedIn·언어 선택을 생략하지 않는다. TA 상세의 마지막 링크는 `../design/index.html`(디자인 포트폴리오), 디자인 상세는 `../index.html`(TA 포트폴리오)이다. 이력서 모달 CSS·JS와 `download,close` 아이콘도 유지한다.

...
- 푸터는 메인과 동일한 전체 링크·1400px 최대 너비·여백을 유지한다. 이메일·이력서·GitHub·LinkedIn·언어 선택을 생략하지 않는다. TA 상세의 마지막 링크는 `../design/index.html`(디자이너 포트폴리오), 디자인 상세는 `../index.html`(TA 포트폴리오)이다. 이력서 모달 CSS·JS와 `download,close` 아이콘도 유지한다.
...
디자인 홈의 6개 프로젝트는 모두 로컬 상세로 연결한다. [디자인 이식 기록](design-portfolio-migration.md)에서 원본·새 경로·데모 위치를 확인한다. 새 상세를 추가하거나 이름을 바꾸면 홈 카드와 이전/다음 링크, 기존 URL의 리다이렉트도 함께 갱신한다.

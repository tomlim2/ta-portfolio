...
- 디자인 홈 `design/index.html`은 같은 카드·About·푸터 형식을 공유하고, 히어로는 소개 문구 없이 데모릴로 시작한다. 디자인 작업 6개를 표시하며 모든 카드를 이식한 로컬 상세 페이지로 연결한다. [이식 기록](design-portfolio-migration.md)을 참고한다.
...
**TA·프론트엔드 홈**의 소개문 아래에는 **tomandlim@gmail.com / 이력서 보기** 두 개의 텍스트 전용 Ghost 링크를 둔다. `.hero-actions`는 위 간격 16px, 링크 사이 가로 간격 24px로 배치하고 좁은 화면에서는 줄바꿈한다. 링크는 공통 `.btn.btn--ghost`의 14px / 20px와 최소 높이 44px를 유지하되 좌우 패딩을 없애 소개문 시작선에 맞춘다. 이메일은 `mailto:`, 이력서는 `data-resume-viewer`로 기존 모달에 연결한다. GitHub·LinkedIn은 홈 푸터에서 제공한다. 디자인 홈은 히어로 액션을 생략하고 이메일·이력서도 푸터에서 제공한다.
...
`.footer-layout`은 1열 grid다. 별도 이름 표기 없이 `.footer-links`에 이메일·이력서·포트폴리오 전환 링크를 왼쪽 정렬한 세로 목록으로 놓고, 그 아래 `.footer-identity`에 저작권을 위 간격 6px로 배치한다. 마지막 링크와 저작권의 글자 중심 간격은 위 링크들 사이와 동일한 32px다. 링크는 패딩·테두리 너비를 0으로 두고 내부도 왼쪽 정렬해 짧은 라벨의 글자 시작점까지 저작권과 맞춘다. `.btn.btn--ghost.footer-link`의 최소 조작 너비는 44px, 높이는 32px다. 링크 사이의 장식 세로 구분선은 사용하지 않는다. 푸터 텍스트·링크는 12px / 16px, 저작권은 12px / 20px다.

하단 `.footer-controls`는 위 간격 32px, 1px 구분선, 위 패딩 20px를 둔다. GitHub·LinkedIn 아이콘은 왼쪽, 언어 버튼은 오른쪽에 배치하며 최소 간격 16px를 둔다. `justify-content: space-between`과 `align-items: center`로 양끝 배치와 세로 중앙 정렬을 유지한다. GitHub·LinkedIn은 모든 화면에서 `.footer-social-links`의 브랜드 아이콘 링크로 제공하고 기존 텍스트 항목은 숨긴다. 아이콘은 16px, 조작 영역은 40×44px이며 접근 가능한 서비스 이름을 갖는다. 푸터 언어 버튼은 128×38px, 글자는 13px / 20px, 지구본·화살표는 16px, 모서리는 10px이며 메뉴는 위로 열린다. 메뉴 항목 높이도 38px로 맞춘다. 현재 언어를 포함한 접근 가능한 이름과 `aria-haspopup`, `aria-expanded`, `aria-controls`를 제공한다.
...
아이콘끼리 모인 행은 `.icon-group`으로 묶는다. 각 `.btn--icon`은 가로 32px · 세로 44px, 추가 gap은 0이며 클릭 영역을 겹치지 않는다. 소셜·툴팁 그룹 예시에 적용한다. 실제 아이콘 크기는 Google 20px 상자 / SNS 16px 도형을 유지한다. 공통 푸터의 소셜 링크는 16px 도형과 40×44px 조작 영역을 사용한다. 단독 아이콘·메뉴는 기본 44×44px를 사용한다. 미리보기 모달 상단은 32×32px 조작 영역과 18px 아이콘을 사용한다.
...
`frontend/index.html`은 TA의 텍스트 히어로·액션 행과 공통 프로젝트 카드·About·푸터를 사용한다. Cork NFT → Winterest → LIKE → Junkyard 순서로 표시한다. 상세는 `projects/`의 공통 896px 본문과 이미지 미리보기를 사용하며, 새 세 상세의 로고·상단 내비게이션은 프론트엔드 홈으로 연결한다. Junkyard는 디자인과 공유한다. 데모·시연 영상·저장소 링크는 해당 제목 옆의 `link_2` 아이콘으로 배치한다. 공통 `btn btn--ghost btn--icon`과 짧은 `data-tip`, 목적지·새 탭을 설명하는 `aria-label`을 사용하며 제목 번역 요소와 링크를 분리한다. 프론트엔드 상세의 이미지·동작 캡처는 모든 화면에서 본문 너비의 1열로 배치하고, figure 사이 간격은 32px로 둔다. 새 UI 변형이나 전용 CSS는 추가하지 않는다.

푸터는 현재 분야를 제외한 나머지 두 포트폴리오로 연결한다. TA에서는 디자이너·프론트엔드, 디자이너에서는 TA·프론트엔드, 프론트엔드에서는 TA·디자이너 링크를 제공한다. 경로·미디어 출처·구현 범위는 [프론트엔드 이식 기록](frontend-portfolio-migration.md)을 따른다.
